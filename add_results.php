        ...
        <form action="" method="post">
            ...
            <legend>Enter Marks</legend>
                ...
                <?php
                    ...
                    $select_class_query="SELECT `Cources` from `classes`";
...
<?php
    if(isset($_POST['rno'],$_POST['p1'],$_POST['p2'],$_POST['p3'],$_POST['p4'],$_POST['p5']))
    {
        ...
        $name=mysqli_query($conn,"SELECT `name` FROM `students` WHERE `rno`='$rno' and `Cource`='$class_name'");
        while($row = mysqli_fetch_array($name)) {
            $display=$row['name'];
            echo $display;
         }

        $sql="INSERT INTO `results` (`name`, `rno`, `Cource`, `s1`, `s2`, `s3`, `s4`, `s5`, `marks`, `percentage`) VALUES ('$display', '$rno', '$class_name', '$p1', '$p2', '$p3', '$p4', '$p5', '$marks', '$percentage')";
